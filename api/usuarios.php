...
if ($method === "POST") {
    ...
    if (isset($input['email']) && isset($input['contrasena']) && !isset($input['nombres'])) {
        ...
        $stmt = $pdo->prepare("SELECT id_usuario, nombres, apellidos, email, contraseña FROM Usuarios WHERE email = :email");
    ...
    if (isset($input['nombres'], $input['apellidos'], $input['email'], $input['contrasena'])) {
        ...
        $stmt = $pdo->prepare("INSERT INTO Usuarios (nombres, apellidos, email, contraseña, id_rol) VALUES (:nombres, :apellidos, :email, :contraseña, :id_rol)");
        $stmt->execute([
            'nombres' => $nombres,
            'apellidos' => $apellidos,
            'email' => $email,
